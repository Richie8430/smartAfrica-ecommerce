...
- Name: webauthn.spec.ts >> WebAuthn / Biometric flows — cancellation and revoke >> Test 3 — Cancellation is handled gracefully
- Location: e2e/tests/webauthn.spec.ts:93:3
...
- main:
  - link "SmartTrade Africa":
    ...
    - img
    - text: SmartTrade Africa
  - heading "Sign in to SmartTrade" [level=1]
  - paragraph: Welcome back — glad to see you!
  - text: Email address*
  ...
  - button "Sign in with fingerprint" [disabled]
  - paragraph:
...
```ts
  22  |       protocol:                    'ctap2',
  23  |       transport:                   'internal',
  24  |       hasResidentKey:              true,
  25  |       hasUserVerification:         true,
  26  |       isUserVerified:              true,
  27  |       automaticPresenceSimulation: true,
  28  |     },
  29  |   });
  30  |   return authenticatorId;
  31  | }
  32  | 
  33  | test.describe.serial('WebAuthn / Biometric flows — enrollment then login', () => {
  34  |   let context: BrowserContext;
  35  |   let page: Page;
  36  |   let cdp: CDPSession;
  37  | 
  38  |   test.beforeAll(async ({ browser }: { browser: Browser }) => {
